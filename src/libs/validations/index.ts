export * from './es-boolean.validations';

export * from './es-estado-mesa.validations';
export * from './es-estado-orden.validations';
export * from './es-nombre.validations';
export * from './es-number-valido.validations';
export * from './es-string-alfabetico.validations';
export * from './es-uuid.validations';
export * from './es-vehiculo.validations';
export * from './longitud-maxima.validations';
export * from './longitud-minima.validations';
